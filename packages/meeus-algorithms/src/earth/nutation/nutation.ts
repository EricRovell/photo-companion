import { getArgumentOfLatitude, getMeanAnomaly, getMeanElongation, getMeanLongitudeAscendingNode } from "../../moon";
...
import type { ArcSecond, Degree, JulianDay } from "../../types";
...
export function getNutationInLongitude (jd: JulianDay): ArcSecond {
	...
	const D = getMeanElongation(jd);
	const M = getMeanAnomaly(jd);
	const Mprime = getMeanAnomaly(jd);
	const F = getArgumentOfLatitude(jd);
	const omega = getMeanLongitudeAscendingNode(jd);
