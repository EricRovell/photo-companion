import { CONSTANT_OF_ABERRATION, DEG2RAD, RAD2DEG } from "../../consts";
import { Sun } from "../../sun";
...
import { getEarthVelocity } from "./earth-velocity";

import type {
	ArcSecond,
	CoordinatesEcliptic,
	CoordinatesEclipticCorrection,
	CoordinatesEquatorialCorrection,
 ...
 * @return {CoordinatesEquatorialCorrection}
 * @memberof module:Earth
 */
export function getAccurateAnnualEquatorialAberration (jd: JulianDay, coords: EquatorialCoordinates): CoordinatesEquatorialCorrection {
	...
	const X0 = velocity.y * cosAlpha - velocity.y * sinAlpha;
 ...
 * @param {JulianDay} jd The julian day
 * @param {EquatorialCoordinates} coords The equatorial coordinates
 * @return {CoordinatesEclipticCorrection} The coordinates corrections in ArcSeconds
 * @memberof module:Earth
 */
export function getAnnualEquatorialAberration (jd: JulianDay, coords: EquatorialCoordinates): CoordinatesEquatorialCorrection {
	const ra = coords.rightAscension * DEG2RAD;
	const dec = coords.declination * DEG2RAD;
	...
	const DeltaRightAscension: ArcSecond = -1 * k * (A0 / cosDelta) + e * k * A1 / cosDelta;
	...
	const DeltaDeclination: ArcSecond = -1 * k * D1 + e * k * D2;
 ...
 * @return {CoordinatesEclipticCorrection} The coordinates corrections in Arcsecond
 * @memberof module:Earth
 */
export function getAnnualEclipticAberration (jd: JulianDay, coords: CoordinatesEcliptic): CoordinatesEclipticCorrection {
	const e = getEccentricity(jd);
	...
	return { DeltaLatitude: Y, DeltaLongitude: X };
}
 ...
 * @memberof module:Earth
 */
export function getNutationEquatorialAberration (jd: JulianDay, coords: EquatorialCoordinates): CoordinatesEquatorialCorrection {
	...
	return { DeltaDeclination, DeltaRightAscension };
}
