import { DEG2RAD, EARTH_EQUATORIAL_RADIUS, EARTH_RADIUS_FLATTENING_FACTOR, RAD2DEG } from "../consts";
import { Sun } from "../sun";
import { Equinox } from "../types";
import { getJulianCentury, getJulianMillennium } from "../utils/julian-day";
...
function getEclipticLongitudeValue (jd: JulianDay, equinox: Equinox = Equinox.MeanOfTheDate): Degree {
	...
	const L0 = (coeffs0).reduce((sum, val) => sum + (val.A * Math.cos(val.B + val.C * tau)), 0);
	const L1 = (coeffs1).reduce((sum, val) => sum + (val.A * Math.cos(val.B + val.C * tau)), 0);
	const L2 = (coeffs2).reduce((sum, val) => sum + (val.A * Math.cos(val.B + val.C * tau)), 0);
	const L3 = (coeffs3).reduce((sum, val) => sum + (val.A * Math.cos(val.B + val.C * tau)), 0);
	const L4 = (coeffs4).reduce((sum, val) => sum + (val.A * Math.cos(val.B + val.C * tau)), 0);
	const L5 = (coeffs5).reduce((sum, val) => sum + (val.A * Math.cos(val.B + val.C * tau)), 0);
...
export function getEclipticLatitude (jd: JulianDay, equinox: Equinox = Equinox.MeanOfTheDate): Degree {
	...
	const B0 = (coeffs0).reduce((sum, val) => sum + (val.A * Math.cos(val.B + val.C * tau)), 0);
	const B1 = (coeffs1).reduce((sum, val) => sum + (val.A * Math.cos(val.B + val.C * tau)), 0);
	const B2 = (coeffs2).reduce((sum, val) => sum + (val.A * Math.cos(val.B + val.C * tau)), 0);
	const B3 = (coeffs3).reduce((sum, val) => sum + (val.A * Math.cos(val.B + val.C * tau)), 0);
	const B4 = (coeffs4).reduce((sum, val) => sum + (val.A * Math.cos(val.B + val.C * tau)), 0);
 ...
 * @param {JulianDay} jd The julian day
 * @param {Equinox} equinox (optional) The equinox to be used: MeanOfTheDate (default) or StandardJ2000.
 * @returns {CoordinatesEcliptic}
 * @memberof module:Earth
 */
export function getCoordinatesEcliptic (jd: JulianDay, equinox: Equinox = Equinox.MeanOfTheDate): CoordinatesEcliptic {
	return {
...
export function getRadiusVector (jd: JulianDay): AstronomicalUnit {
	...
	const R0 = (coeffs0).reduce((sum, val) => sum + (val.A * Math.cos(val.B + val.C * tau)), 0);
	const R1 = (coeffs1).reduce((sum, val) => sum + (val.A * Math.cos(val.B + val.C * tau)), 0);
	const R2 = (coeffs2).reduce((sum, val) => sum + (val.A * Math.cos(val.B + val.C * tau)), 0);
	const R3 = (coeffs3).reduce((sum, val) => sum + (val.A * Math.cos(val.B + val.C * tau)), 0);
	const R4 = (coeffs4).reduce((sum, val) => sum + (val.A * Math.cos(val.B + val.C * tau)), 0);
