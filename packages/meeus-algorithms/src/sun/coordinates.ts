import { DEG2RAD, STANDARD_ALTITUDE_SUN } from "../consts";
import { getCorrectionInLatitude, getCorrectionInLongitude, transformEclipticToEquatorial } from "../coordinates";
import { getEclipticLatitude, getEclipticLongitude, getMeanObliquityOfEcliptic, getNutationInLongitude, getRadiusVector, getTrueObliquityOfEcliptic } from "../earth";
import { getRiseTransitSetTimes } from "../risetransitset";
import { Equinox } from "../types";
import { getJulianCentury, getJulianDayMidnightDynamicalTime } from "../utils/julian-day";
import { fmod360 } from "../utils/math";
import { getDecimalValue } from "../utils/sexagesimal";
...
export function getGeocentricEclipticLongitude (jd: JulianDay, equinox: Equinox = Equinox.MeanOfTheDate): Degree {
	return fmod360(getEclipticLongitude(jd, equinox) + 180);
}
...
export function getGeocentricEclipticLatitude (jd: JulianDay, equinox: Equinox = Equinox.MeanOfTheDate): Degree {
	return -getEclipticLatitude(jd, equinox);
}
 ...
 */
export function getGeocentricCoordinatesEcliptic (jd: JulianDay, equinox: Equinox = Equinox.MeanOfTheDate): CoordinatesEcliptic {
	return {
...
export function getGeocentricEquatorialCoordinates (jd: JulianDay, equinox: Equinox = Equinox.MeanOfTheDate): EquatorialCoordinates {
	return transformEclipticToEquatorial(getGeocentricCoordinatesEcliptic(jd, equinox), getMeanObliquityOfEcliptic(jd));
}
...
export function getApparentGeocentricEclipticLongitude (jd: JulianDay): Degree {
	...
	Longitude = Longitude + getDecimalValue(0, 0, getNutationInLongitude(jd));
 ...
 * @returns {CoordinatesEcliptic}
 */
export function getApparentGeocentricCoordinatesEcliptic (jd: JulianDay): CoordinatesEcliptic {
	return {
...
export function getApparentGeocentricEquatorialCoordinates (jd: JulianDay): EquatorialCoordinates {
	return transformEclipticToEquatorial(getApparentGeocentricCoordinatesEcliptic(jd), getTrueObliquityOfEcliptic(jd));
}
 ...
 * @param {CoordinatesGeographic} geoCoords The observer's location.
 * @returns {RiseTransitSet}
 */
export function getRiseTransitSet (jd: JulianDay, geoCoords: CoordinatesGeographic): RiseTransitSet {
