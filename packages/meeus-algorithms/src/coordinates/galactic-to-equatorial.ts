import { DEG2RAD, J2000, JULIAN_DAY_B1950_0, RAD2DEG } from "../consts";
import { fmod360, fmod90 } from "../utils/math";
import { precessEquatorialCoordinates } from "./precession";

import type { CoordinatesEcliptic as CoordinatesGalactic, Degree, EquatorialCoordinates, JulianDay } from "../types";

/**
 * Equatorial right ascension in epoch B1950 from galactic coordinates. See AA p.94
 *
 * @param {CoordinatesGalactic } coords The galactic coordinates
 * @returns {Degree}
 */
export function getEquatorialRightAscensionB1950FromGalactic (coords: CoordinatesGalactic): Degree {
 ...
 * @returns {Degree}
 */
export function getEquatorialDeclinationB1950FromGalactic (coords: CoordinatesGalactic): Degree {
	const c2 = 27.4 * DEG2RAD;
	const lprime = (coords.longitude - 123) * DEG2RAD;
	const b = coords.latitude * DEG2RAD;
 ...
 */
export function transformGalacticToEquatorial (coords: CoordinatesGalactic, epoch: JulianDay = J2000): EquatorialCoordinates {
	const coordsB1950 = {
