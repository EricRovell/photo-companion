...
import type { Day, JulianDay, Month, Year } from "../types";
...
export function getFullScaleJulianDay (Year: Year, Month: Month, Day: Day, isGregorianCalendar = true): JulianDay {
	if (Month < 3) {
		Year = Year - 1;
		Month = Month + 12;
	}
...
export function getFractionalYear (jd: JulianDay, isGregorianCalendar = true): number {
...
export function getDecimalYear (jd: JulianDay, isGregorianCalendar = true): number {
	const year = getDate(jd).getFullYear();
	const daysInYear = (isLeapYear(year, isGregorianCalendar)) ? 366 : 365;
