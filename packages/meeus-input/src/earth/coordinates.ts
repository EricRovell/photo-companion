...
import { getJulianCentury, getJulianMillennium } from "../juliandays";
import { Sun } from "../sun";
import {
	Equinox
} from "../types";
import { fmod360, fmod90 } from "../utils";
import * as COEFFICIENTS from "./coefficients";

import type { AstronomicalUnit, Degree, EclipticCoordinates, JulianDay, Meter, Radian } from "../types";
...
function getEclipticLongitudeValue (jd: JulianDay, equinox: Equinox = Equinox.MeanOfTheDate): Degree {
	...
	const coeffs0 = COEFFICIENTS.L0;
	const coeffs1 = (equinox === Equinox.MeanOfTheDate) ? COEFFICIENTS.L1 : COEFFICIENTS.L1_J2000;
	const coeffs2 = (equinox === Equinox.MeanOfTheDate) ? COEFFICIENTS.L2 : COEFFICIENTS.L2_J2000;
	const coeffs3 = (equinox === Equinox.MeanOfTheDate) ? COEFFICIENTS.L3 : COEFFICIENTS.L3_J2000;
	const coeffs4 = (equinox === Equinox.MeanOfTheDate) ? COEFFICIENTS.L4 : COEFFICIENTS.L4_J2000;
	const coeffs5 = (equinox === Equinox.MeanOfTheDate) ? COEFFICIENTS.L5 : [];

	const L0 = (coeffs0).reduce((sum, val) => sum + (val[0] * Math.cos(val[1] + val[2] * tau)), 0);
	const L1 = (coeffs1).reduce((sum, val) => sum + (val[0] * Math.cos(val[1] + val[2] * tau)), 0);
	const L2 = (coeffs2).reduce((sum, val) => sum + (val[0] * Math.cos(val[1] + val[2] * tau)), 0);
	const L3 = (coeffs3).reduce((sum, val) => sum + (val[0] * Math.cos(val[1] + val[2] * tau)), 0);
	const L4 = (coeffs4).reduce((sum, val) => sum + (val[0] * Math.cos(val[1] + val[2] * tau)), 0);
	const L5 = (coeffs5).reduce((sum, val) => sum + (val[0] * Math.cos(val[1] + val[2] * tau)), 0);
...
export function getEclipticLatitude (jd: JulianDay, equinox: Equinox = Equinox.MeanOfTheDate): Degree {
	...
	const coeffs0 = (equinox === Equinox.MeanOfTheDate) ? COEFFICIENTS.B0 : COEFFICIENTS.B0;
	const coeffs1 = (equinox === Equinox.MeanOfTheDate) ? COEFFICIENTS.B1 : COEFFICIENTS.B1_J2000;
	const coeffs2 = (equinox === Equinox.MeanOfTheDate) ? [] : COEFFICIENTS.B2_J2000;
	const coeffs3 = (equinox === Equinox.MeanOfTheDate) ? [] : COEFFICIENTS.B3_J2000;
	const coeffs4 = (equinox === Equinox.MeanOfTheDate) ? [] : COEFFICIENTS.B4_J2000;

	const B0 = (coeffs0).reduce((sum, val) => sum + (val[0] * Math.cos(val[1] + val[2] * tau)), 0);
	const B1 = (coeffs1).reduce((sum, val) => sum + (val[0] * Math.cos(val[1] + val[2] * tau)), 0);
	const B2 = (coeffs2).reduce((sum, val) => sum + (val[0] * Math.cos(val[1] + val[2] * tau)), 0);
	const B3 = (coeffs3).reduce((sum, val) => sum + (val[0] * Math.cos(val[1] + val[2] * tau)), 0);
	const B4 = (coeffs4).reduce((sum, val) => sum + (val[0] * Math.cos(val[1] + val[2] * tau)), 0);
...
export function getRadiusVector (jd: JulianDay): AstronomicalUnit {
	...
	const coeffs0 = COEFFICIENTS.R0;
	const coeffs1 = COEFFICIENTS.R1;
	const coeffs2 = COEFFICIENTS.R2;
	const coeffs3 = COEFFICIENTS.R3;
	const coeffs4 = COEFFICIENTS.R4;

	const R0 = (coeffs0).reduce((sum, val) => sum + (val[0] * Math.cos(val[1] + val[2] * tau)), 0);
	const R1 = (coeffs1).reduce((sum, val) => sum + (val[0] * Math.cos(val[1] + val[2] * tau)), 0);
	const R2 = (coeffs2).reduce((sum, val) => sum + (val[0] * Math.cos(val[1] + val[2] * tau)), 0);
	const R3 = (coeffs3).reduce((sum, val) => sum + (val[0] * Math.cos(val[1] + val[2] * tau)), 0);
	const R4 = (coeffs4).reduce((sum, val) => sum + (val[0] * Math.cos(val[1] + val[2] * tau)), 0);
