...
import { Earth, juliandays, Sun, times } from "../../src";
import { H2DEG } from "../../src/constants";
import { MOON_SYNODIC_PERIOD } from "../../src/moon/consts";
...
describe("moon", () => {
	...
	test("get time of new moon", () => {
		...
		const newMoonJD = Earth.Moon.getTimeOfMeanPhase(juliandays.getJulianDay(UTCDate), "NEW");
	...
	test("get moon age", () => {
		...
		const fqMoonJD = Earth.Moon.getTimeOfMeanPhase(juliandays.getJulianDay(UTCDate), "FIRST_QUARTER");
		expect(Earth.Moon.getAge(fqMoonJD)).toBeCloseTo(MOON_SYNODIC_PERIOD / 4, 6);
		const fullMoonJD = Earth.Moon.getTimeOfMeanPhase(juliandays.getJulianDay(UTCDate), "FULL");
		expect(Earth.Moon.getAge(fullMoonJD)).toBeCloseTo(MOON_SYNODIC_PERIOD / 2, 5);
		const lqMoonJD = Earth.Moon.getTimeOfMeanPhase(juliandays.getJulianDay(UTCDate), "LAST_QUARTER");
	...
	test("get moon age name", () => {
		const UTCDate = new Date(Date.UTC(1977, 1, 12));
		const newMoonJD = Earth.Moon.getTimeOfMeanPhase(juliandays.getJulianDay(UTCDate), "NEW");
		expect(Earth.Moon.getAgeName(newMoonJD)).toEqual("NEW");
		const fqMoonJD = Earth.Moon.getTimeOfMeanPhase(juliandays.getJulianDay(UTCDate), "FIRST_QUARTER");
		expect(Earth.Moon.getAgeName(fqMoonJD)).toEqual("FIRST_QUARTER");
		const fullMoonJD = Earth.Moon.getTimeOfMeanPhase(juliandays.getJulianDay(UTCDate), "FULL");
		expect(Earth.Moon.getAgeName(fullMoonJD)).toEqual("FULL");
		const lqMoonJD = Earth.Moon.getTimeOfMeanPhase(juliandays.getJulianDay(UTCDate), "LAST_QUARTER");
		expect(Earth.Moon.getAgeName(lqMoonJD)).toEqual("LAST_QUARTER");
	});
});
