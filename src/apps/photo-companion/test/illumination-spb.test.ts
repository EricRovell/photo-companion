...
import { scheduleLightsSPb as data } from "@shared/schedule";
...
const testFn = (month: number, date: number) => (...output: number[]) => {
...
describe("Illumination schedule, Saint-Petersburg, Russia", () => {
	...
	describe("Nearest illumination event", () => {
		...
		it("The time is before the lights end today", () => {
			const state = getLightsStateByDateSpb(new Date(CURRENT_YEAR, 7, 31, 4, 15));
			expect(state).toEqual({
				lights: true,
				event: "lights:end",
				timestamp: new Date(CURRENT_YEAR, 7, 31, 5, 20).getTime()
			});
		});
		it("The time is before the lights start today", () => {
			const state = getLightsStateByDateSpb(new Date(CURRENT_YEAR, 7, 31, 10, 20));
			expect(state).toEqual({
				lights: false,
				event: "lights:start",
				timestamp: new Date(CURRENT_YEAR, 7, 31, 20, 30).getTime()
			});
		});
		it("The time is during the lights today, they will turn off the next day", () => {
			const state = getLightsStateByDateSpb(new Date(CURRENT_YEAR, 7, 31, 21, 20));
			expect(state).toEqual({
				lights: true,
				event: "lights:end",
				timestamp: new Date(CURRENT_YEAR, 8, 1, 5, 35).getTime()
			});
		});
	});
});
